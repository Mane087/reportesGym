...
const fetchData = async () => {
    ...
    data.value = await getDataByTrainer(trainer);
...
const aggregatedData = computed(() => {
    ...
    filteredData.value.forEach((item) => {
        ...
        if (!aggregated[month][sucursal]) {
            aggregated[month][sucursal] = 0;
        }

        aggregated[month][sucursal] += item._count.id_usuario;
    });
    ...
    <div class="w-full h-max overflow-hidden">
        <div class="w-[90%] h-max mx-auto py-5 flex flex-row justify-evenly items-center">
            <div class="h-full flex flex-row items-center gap-x-2">
                ...
                <button class="px-2 rounded-md bg-blue-700 text-white" @click="fetchData">Buscar</button>
            </div>
        </div>
            ...
            <table class="w-full h-full mb-5">
                <thead>
                    <tr>
                        ...
                        <th class="border border-[#ccc]">Olympus</th>
                ...
                <tbody>
                    <tr v-for="month in getFilteredMonths()" :key="month.value">
                        ...
                        <td class="border border-[#ccc]">{{ aggregatedData[month.value] ?
                            aggregatedData[month.value][34] || 0 : 0 }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
